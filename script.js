...
document.addEventListener('keydown', (e) => {
    if (
        ...
        if (
            [
                'KeyL', 'KeyJ', 'ArrowLeft', 'ArrowRight',
                'Period', 'Comma', 'KeyA'
            ].includes(e.code) ||
            (
                e.code === 'KeyS' &&
        ...
        if (['KeyJ', 'KeyL', 'ArrowLeft', 'ArrowRight', 'Comma', 'Period'].includes(e.code)) {
            ...
            if (['Digit0', 'Digit1', 'Digit2', 'Digit3', 'Digit4', 'Digit5', 'Digit6', 'Digit7', 'Digit8', 'Digit9'].includes(e.code)) {
                ...
                if (document.querySelector('a.yt-simple-endpoint.style-scope.ytd-menu-navigation-item-renderer')) {
                    document.querySelector('a.yt-simple-endpoint.style-scope.ytd-menu-navigation-item-renderer').click()
                } else {
...
window.addEventListener("keydown", function (event) {
    ...
    displaySpeed(video?.playbackRate, previousSpeed);
});
...
function displaySpeed(speed = 1, previousSpeed = 1, text = "") {
...
function initializeExtensionButton() {
    ...
    if (location.pathname.startsWith("/shorts/")) {
        getTargetElements((targetElement, autoplayTargetElement) => {
            if (!document.getElementById("open-extension-shorts")) {
                const imageURL = chrome.runtime.getURL("src/images/icon.png");

                const buttonHTML = `
                <button id="open-extension-shorts"
                ...
                const openButton = document.getElementById("open-extension-shorts");
        ...
        if (!document.getElementById("open-extension")) {
            const imageURL = chrome.runtime.getURL("src/images/yt-movie-icon.png");

            const buttonHTML = `
            <button id="open-extension"
                class="ytp-button"
                aria-expanded="false"
                aria-haspopup="true"
        ...
        `;
            ...
            const openButton = document.getElementById("open-extension");
...
function updateTimeDisplay(rateBasedTimeDisplay, display = true) {
    ...
    if (rateBasedTimeDisplay === true) {
        ...
        if (display) displaySpeed(video?.playbackRate, video?.playbackRate, "Smart⏱");
        ...
        if (display) displaySpeed(video?.playbackRate, video?.playbackRate, "Real ⏱");
        if (timeDisplayInterval) {
...
function toggleAutoPlay(isShorts = true, clicked = false) {
    if (isShorts) {
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
            chrome.storage.sync.get('autoplay', (result) => {
                if (typeof result.autoplay === 'boolean') {
                    ...
                    updateButton(document.getElementById('autoplay-button'), !result.autoplay);
                    displaySpeedLabel();
                    displaySpeed(video?.playbackRate, video?.playbackRate, `Auto ${!result.autoplay ? '▶' : '⏸'}`);
                    if (result.autoplay) {
                        video.loop = true;
                        video.dataset.STLoop = "true";
                        video.removeEventListener('ended', nextShort);
                    ...
                    displaySpeedLabel();
                    displaySpeed(video?.playbackRate, video?.playbackRate, 'Auto ▶');
        ...
        displaySpeed(video?.playbackRate, video?.playbackRate, `Auto ${document.querySelector('.ytp-autonav-toggle')?.querySelector('.ytp-autonav-toggle-button-container')?.querySelector('.ytp-autonav-toggle-button')?.ariaChecked === "true" ? '▶' : '⏸'}`);
    }
}
